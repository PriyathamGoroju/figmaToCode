...
function iterateLayers(node: BaseNode, htmlCode: string[]): void {
  if ('children' in node) {
    let elementStartTag = '';
    if (node.type === 'FRAME') {
...
function generateElementStartTag(node: FrameNode): string {
...
function generateNodeHTMLCode(node: BaseNode): string {
  let nodeHTMLCode = '';

  if (node.type === 'RECTANGLE') {
    if (hasImageFill(node as RectangleNode)) {
...
function getCornerRadius(node: PolygonNode): number {
...
function generatePolygonPoints(cornerRadius: number, width: number, height: number, sides: number): string {
...
function getBackgroundColor(node: RectangleNode | PolygonNode | EllipseNode | FrameNode): string | undefined {
...
function getImageURL(node: RectangleNode): string | null {
  const fills = node.fills as readonly Paint[];
  const imageFills = fills.filter((fill) => fill.type === 'IMAGE');

  const imageUrl= (imageFills[0] as ImagePaint).imageHash ?? '';
  const y = figma.getImageByHash(imageUrl);
  console.log(y)
  const workableImageUrl = figma.getImageByHash(imageUrl)?.getBytesAsync.name;
